...
//Rare exceptions to indicate a bug in the _mapappweek4 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_mapappweek4 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_mapappweek4]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.mapappweek4.module"
