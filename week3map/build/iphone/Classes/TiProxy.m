...
//Rare exceptions to indicate a bug in the _week3map code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_week3map = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_week3map]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.week3map.module"
